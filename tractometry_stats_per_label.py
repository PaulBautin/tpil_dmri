...
def get_parser():
    ...
    mandatory.add_argument(
        ...
        help='Path to folder that contains output .xlsx files (e.g. "22-07-13_tractometry_DCL/Statistics")',
    )
    mandatory.add_argument(
        "-con",
        required=True,
...
def load_data(file):
    """
    Creates a dataframe based on information present on tractometry output xlsx files
...
def df_gather_metrics(df, group_name, axis_name):
    ...
    :param df: 'load_data' output dataframe. Ex: 'mean_std.xlsx'
...
def compute_pca(df_con, df_dcl, labels, diff=None, output_metrics=['PCA_1', 'PCA_2']):
    if diff is None:
        df = pd.concat([df_con, df_dcl]).dropna().groupby(labels).mean()
    if diff == "per_tract":
        df_con_copy = df_con.groupby(["tracts"]).sample(n=1000, replace=True).drop(
            ["subjects", "group_name", "points"], axis=1).set_index("tracts")
        df_dcl_copy = df_dcl.groupby(["tracts"]).sample(n=1000, replace=True).drop(
            ["subjects", "group_name", "points"], axis=1).set_index("tracts")
        df = df_con_copy.subtract(df_dcl_copy).dropna()
    if diff == "per_point":
        df_con_copy = df_con.groupby(["tracts", "points"]).sample(n=1000, replace=True).drop(
            ["subjects", "group_name"], axis=1).set_index(["tracts","points"])
        df_dcl_copy = df_dcl.groupby(["tracts", "points"]).sample(n=1000, replace=True).drop(
            ["subjects", "group_name"], axis=1).set_index(["tracts","points"])
        df = df_con_copy.subtract(df_dcl_copy).dropna()
...
def t_test(df_con, df_dcl, path_output, labels, pca=False, per_tract=False):
    if per_tract is False:
        df_dcl = df_dcl.set_index(labels).drop(["subjects", "group_name"], axis=1)
        df_con = df_con.set_index(labels).drop(["subjects", "group_name"], axis=1)
    elif per_tract:
        df_dcl = df_dcl.groupby([labels, "subjects"]).mean().reset_index().set_index(labels).drop(["subjects"], axis=1)
        df_con = df_con.groupby([labels, "subjects"]).mean().reset_index().set_index(labels).drop(["subjects"], axis=1)
    ...
    for metric in metrics:
        for ind in df_results.index:
            ...
            df_results.loc[ind, metric + "_p"] = p_value
    return df_results


def lineplot_t_test_per_point(df_metrics, bundle="OR_ML_R"):
    df_metrics = df_metrics.loc[bundle]
...
def lineplot_per_point(df_con, df_dcl, bundle="AF_L", metric='fa_metric_mean'):
    df_con = df_con.loc[bundle]
    df_dcl = df_dcl.loc[bundle]
    plt.title("Average metric across dcl and control subjects")
    plt.xlabel("position on bundle " + bundle)
    plt.ylabel(metric)
    sns.lineplot(data=df_con[metric])
    sns.lineplot(data=df_con[metric])
    plt.show()
...
def main():
    ...
    path_results_dcl = os.path.abspath(os.path.expanduser(arguments.dcl))
    path_results_con = os.path.abspath(os.path.expanduser(arguments.con))
    path_output = os.path.abspath(arguments.o)

    ## Form main Dataframes df_metrics_con for control subjects and df_metric_dcl for DCL
    # CON subjects
    df_mean_std_con = load_data(os.path.join(path_results_con, r'mean_std_per_label.xlsx'))
    df_metrics_con = df_gather_metrics(df_mean_std_con, group_name="con", axis_name=["subjects", "tracts"])
    df_metrics_con[['tracts', 'points']] = df_metrics_con['tracts'].str.rsplit('_', 1, expand=True)

    # DCL subjects
    df_mean_std_dcl = load_data(os.path.join(path_results_dcl, r'mean_std_per_label.xlsx'))
    df_metrics_dcl = df_gather_metrics(df_mean_std_dcl, group_name="dcl", axis_name=["subjects", "tracts"])
    df_metrics_dcl[['tracts', 'points']] = df_metrics_dcl['tracts'].str.rsplit('_', 1, expand=True)

    ## Compute PCA metrics
    # Compute PCA metrics per tract
    # df_pca_per_tract, pca_per_tract, df_x_norm_per_tract, = compute_pca(df_metrics_con, df_metrics_dcl, labels=["tracts", "subjects", "group_name"])
    # df_pca_per_tract_diff, pca_per_tract_diff, df_x_norm_per_tract_diff, = compute_pca(df_metrics_con, df_metrics_dcl, labels=["tracts"], diff="per_tract")
    ...
    df_pca_per_point, pca_per_point, df_x_norm_per_point, = compute_pca(df_metrics_con, df_metrics_dcl,
                                                                        labels=["tracts", "subjects", "points",
                                                                                "group_name"])
    df_pca_per_point_con = df_pca_per_point[df_pca_per_point["group_name"] == "con"]
    df_pca_per_point_dcl = df_pca_per_point[df_pca_per_point["group_name"] == "dcl"]

    # Add PCA metrics per point of difference between control and DCL
    df_pca_per_point_diff, pca_per_point_diff, df_x_norm_per_point_diff, = compute_pca(df_metrics_con, df_metrics_dcl,
                                                                                       labels=["tracts"],
                                                                                       diff="per_point",
    ...
    df_pca_per_point_inv_con = df_pca_per_point_inv[df_pca_per_point_inv["group_name"] == "con"]
    df_pca_per_point_inv_dcl = df_pca_per_point_inv[df_pca_per_point_inv["group_name"] == "dcl"]


    # Add metrics PCA and PCA_diff to main dataframe
    df_metrics_con = df_metrics_con.merge(df_pca_per_point_con, how="left",
                                          on=["tracts", "subjects", "points", "group_name"])
    df_metrics_dcl = df_metrics_dcl.merge(df_pca_per_point_dcl, how="left",
                                          on=["tracts", "subjects", "points", "group_name"])
    df_metrics_con = df_metrics_con.merge(df_pca_per_point_inv_con, how="left",
                                          on=["tracts", "subjects", "points", "group_name"])
    df_metrics_dcl = df_metrics_dcl.merge(df_pca_per_point_inv_dcl, how="left",
                                          on=["tracts", "subjects", "points", "group_name"])

    ### Stats
    ## Stats per point
    # t_test between Control and DCL
    df_t_test_per_point = t_test(df_metrics_con, df_metrics_dcl, path_output, labels=["tracts", "points"])
    ...
    print('[')
    for val in df_t_test_per_point.loc["AF_L"]["fa_metric_mean_p"].values:
        print('{},'.format(val))
    print(']')
    
    ## Stats per tract
    # t_test between Control and DCL
    df_t_test_per_tract = t_test(df_metrics_con, df_metrics_dcl, path_output, labels="tracts", per_tract=True)
    ...
    ## Figures
    lineplot_per_point(df_metrics_con, df_metrics_dcl, bundle="AF_L", metric='fa_metric_mean')
    lineplot_t_test_per_point(df_t_test_per_point, bundle="AF_L")
