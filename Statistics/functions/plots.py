...
def boxplot_intersubject_per_ses(df, metric='nufo', bundle="NAC_mPFC_L_27"):
    ...
    axs[1].set_title("Boxplot of "+ metric + " for dcl and control subjects for each time point "+bundle+"\nv2")
    sns.violinplot(data=df_v3, x="group_name", y=metric, palette="CMRmap_r", ax=axs[2])
    axs[2].set_title('v3')
    sns.violinplot(data=df, x="group_name", y=metric, palette="CMRmap_r", ax=axs[3])
...
def lineplot_per_point(df, metric='nufo', bundle="NAC_mPFC_L_27"):
    ...
    axs[0].set_title(metric + " for dcl and control subjects along the bundle "+bundle+"\n v1")
