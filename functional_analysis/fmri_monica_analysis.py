...
def main():
    ...
    df_img = df_img[df_img.subject.isin(['sub-06'])]
    df_img = df_img.drop(columns=['img_func_original'])

    # load segmentations
    df_img['subseg'] =  df_img.apply(lambda x : nib.Nifti1Image((x.img_subseg.get_fdata() == 26).astype(int), x.img_subseg.affine, dtype=np.int32), axis=1)
    print(df_img)

    # load confounds
    #df_img['confounds'] =  df_img.apply(lambda x : load_confounds_strategy(x.path_func_original, denoise_strategy='simple'), axis=1)

    # extract and clean signal for cortex 
    ...
    df_img['clean']=  df_img.apply(lambda x : signal.clean(x.ext, detrend=True, standardize=True, low_pass=0.15, high_pass=0.01, t_r=1.075).T, axis=1)
    df_img = df_img.drop(columns=['ext'])

    # extract signal from subcortex region
    df_img['ext_sub'] =  df_img.apply(lambda x : NiftiLabelsMasker(x.subseg).fit_transform(x.img_func.slicer[:,:,:,:slices]), axis=1) 
    df_img['clean_sub']=  df_img.apply(lambda x : signal.clean(x.ext_sub, detrend=True, standardize=True, low_pass=0.15, high_pass=0.01, t_r=1.075).T, axis=1)
    df_img = df_img.drop(columns=['ext_sub'])


    # metge cleaned signal and create connectivity matrix
    df_img['clean_tot'] =  df_img.apply(lambda x : np.vstack((x.clean, x.clean_sub)), axis=1)
    df_img = df_img.drop(columns=['clean', 'clean_sub'])
    df_img['adj'] =  df_img.apply(lambda x : np.abs(connectome.ConnectivityMeasure(kind='correlation').fit_transform([x.clean_tot.T])[0]), axis=1)
    ...
    p.add_layer(df_img['surf_data_func'].mean(), cbar=True, cmap='viridis')
    fig = p.build()
    plt.show()
    ...
    #df_img['conn_match'] = df_img.apply(lambda x: match_length_degree_distribution(x.img_commit2, x.img_len)[1], axis=1)
    #print(df_img)
    ...
    for id in ['sc', 'commit2', 'len']:
        ...
        lh, rh = surfaces['inflated']
        p = Plot(lh, rh, views=['lateral','medial'])
        p.add_layer(df_img[id + metric].mean(), cbar=True, cmap='plasma')
        fig = p.build()
        plt.show()     
    ...
    #df_img = df_img.drop(columns=['rotated', 'surf_data_func', 'adj', 'clean_tot', 'clean', 'ext', 'img_func', 'img_func_original', 'confounds'])
    df_img.to_pickle('/home/pabaua/dev_tpil/data/correlations_ohbm_compcorr.pkl')
    #df_img = pd.read_pickle('/home/pabaua/dev_tpil/data/correlations_ohbm_compcorr.pkl')
    print(df_img['commit2_sp'])
