...
# Compare filtering
network_1 = np.mean(conn_clbp, where=conn_clbp>0, axis=2)
...
# plt.show()
# plot_point_brain(data=z_score, coords=centroids, cbar=True, views_orientation='horizontal', views_size=(5, 4))
# plt.show()
surf_z_score = map_to_labels(z_score, atlas_labels, mask=atlas_labels > 0, fill=np.nan)
print(surf_z_score)
# plot_point_brain(data=surf_z_score, coords=surfaces, cbar=True, views_orientation='horizontal', views_size=(5, 4), size=1)
# plt.show()
lhsurf = mesh.mesh_io.read_surface(lhsurface)
rhsurf = mesh.mesh_io.read_surface(rhsurface)
plot_hemispheres(lhsurf, rhsurf, surf_z_score)
...
plot_hemispheres(lhsurf, rhsurf, nsynth_labels)
print(nsynth)
...
rotated = nulls.moran(surf_z_score, atlas='fsLR', density='32k', n_perm=1000, seed=1234, parcellation=(nib.load(lhlabels), nib.load(rhlabels)))
print(rotated[2].shape)
corr, pvalue = stats.compare_images(surf_z_score, nsynth, nulls=rotated)
